...
describe('SendToHashMock contract', () => {
   ...
   it ('properly handles internal mappings when invoking valid revert() for coin', async () => {
      ...
      await sendToHashMock.revertPayment('a', ASSET_TYPE_COIN, ZERO_ADDRESS)
   ...
   it ('properly handles internal mappings when invoking valid revert() for token', async () => {
      ...
      await sendToHashMock.connect(signer2).revertPayment('a', ASSET_TYPE_TOKEN, mockToken.address)

      expect(await sendToHashMock.getBeneficiaryPayersMap(ownerAddress, 'a', ASSET_TYPE_TOKEN, mockToken.address))
         .to.be.equal(true)
      expect(await sendToHashMock.getBeneficiaryPayersMap(signer2Address, 'a', ASSET_TYPE_TOKEN, mockToken.address))
         .to.be.equal(false)
      expect(await sendToHashMock.getBeneficiaryPayersMap(ownerAddress, 'b', ASSET_TYPE_TOKEN, mockToken.address))
         .to.be.equal(false)
      ...
      expect(await sendToHashMock.getPayerAssetMapAmount(signer2Address, 'a', ASSET_TYPE_TOKEN, mockToken.address))
         .to.be.equal(0)
      expect((await sendToHashMock.getPayerAssetMapAssetIds(signer2Address, 'a', ASSET_TYPE_TOKEN, mockToken.address)).length)
         .to.be.equal(0)
      expect(await sendToHashMock.getBeneficiaryMapAmount('a', ASSET_TYPE_TOKEN, mockToken.address)).to.be.equal(75)
      expect((await sendToHashMock.getBeneficiaryMapAssetIds('a', ASSET_TYPE_TOKEN, mockToken.address, signer2Address)).length)
         .to.be.equal(0)
      expect((await sendToHashMock.getBeneficiaryMapAssetIds('a', ASSET_TYPE_TOKEN, mockToken.address, ownerAddress)).length)
         .to.be.equal(0)

      await sendToHashMock.connect(signer2).revertPayment('b', ASSET_TYPE_TOKEN, mockToken.address)

      expect(await sendToHashMock.getPayerAssetMapAmount(signer2Address, 'b', ASSET_TYPE_TOKEN, mockToken.address))
         .to.be.equal(0)
      expect((await sendToHashMock.getPayerAssetMapAssetIds(signer2Address, 'b', ASSET_TYPE_TOKEN, mockToken.address)).length)
         .to.be.equal(0)
      expect(await sendToHashMock.getBeneficiaryMapAmount('b', ASSET_TYPE_TOKEN, mockToken.address)).to.be.equal(0)
      expect((await sendToHashMock.getBeneficiaryMapAssetIds('b', ASSET_TYPE_TOKEN, mockToken.address, signer2Address)).length)
         .to.be.equal(0)
      expect((await sendToHashMock.getBeneficiaryMapAssetIds('b', ASSET_TYPE_TOKEN, mockToken.address, ownerAddress)).length)
         .to.be.equal(0)
      ...
      await sendToHashMock.revertPayment('a', ASSET_TYPE_TOKEN, mockToken.address)

      expect(await sendToHashMock.getPayerAssetMapAmount(ownerAddress, 'a', ASSET_TYPE_TOKEN, mockToken.address))
         .to.be.equal(0)
      expect(await sendToHashMock.getPayerAssetMapAmount(signer2Address, 'a', ASSET_TYPE_TOKEN, mockToken.address))
         .to.be.equal(0)
      expect((await sendToHashMock.getPayerAssetMapAssetIds(ownerAddress, 'a', ASSET_TYPE_TOKEN, mockToken.address)).length)
         .to.be.equal(0)
      expect(await sendToHashMock.getBeneficiaryMapAmount('a', ASSET_TYPE_TOKEN, mockToken.address)).to.be.equal(0)
      expect((await sendToHashMock.getBeneficiaryMapAssetIds('a', ASSET_TYPE_TOKEN, mockToken.address, signer2Address)).length)
         .to.be.equal(0)
      expect((await sendToHashMock.getBeneficiaryMapAssetIds('a', ASSET_TYPE_TOKEN, mockToken.address, ownerAddress)).length)
         .to.be.equal(0)
   ...
   it ('properly handles internal mappings when invoking valid revert() for NFT', async () => {
      ...
      await sendToHashMock.connect(signer2).sendToAnyone('b', 1, ASSET_TYPE_NFT, mockNFT.address, 1, {value: dollarInWei})
      await sendToHashMock.sendToAnyone('a', 1, ASSET_TYPE_NFT, mockNFT.address, 3, {value: dollarInWei})
      await sendToHashMock.sendToAnyone('a', 1, ASSET_TYPE_NFT, mockNFT.address, 4, {value: dollarInWei})
      await sendToHashMock.sendToAnyone('a', 1, ASSET_TYPE_NFT, mockNFT.address, 5, {value: dollarInWei})
      await sendToHashMock.connect(signer2).sendToAnyone('a', 1, ASSET_TYPE_NFT, mockNFT.address, 2, {value: dollarInWei})

      await sendToHashMock.connect(signer2).revertPayment('a', ASSET_TYPE_NFT, mockNFT.address)
      ...
      expect((await sendToHashMock.getPayerAssetMapAssetIds(signer2Address, 'a', ASSET_TYPE_NFT, mockNFT.address)).length)
         .to.be.equal(0)
      expect(await sendToHashMock.getBeneficiaryMapAmount('a', ASSET_TYPE_NFT, mockNFT.address)).to.be.equal(3)
      expect((await sendToHashMock.getBeneficiaryMapAssetIds('a', ASSET_TYPE_NFT, mockNFT.address, signer2Address)).length)
         .to.be.equal(0)
      expect(await sendToHashMock.getBeneficiaryMapAssetIds('a', ASSET_TYPE_NFT, mockNFT.address, ownerAddress))
      ...
      expect(await sendToHashMock.getBeneficiaryPayersMap(signer2Address, 'a', ASSET_TYPE_NFT, mockNFT.address))
         .to.be.equal(false)
      expect(await sendToHashMock.getBeneficiaryPayersMap(signer2Address, 'b', ASSET_TYPE_NFT, mockNFT.address))
         .to.be.equal(true)
   ...
   it ('properly handles internal mappings when invoking valid claim() for coin', async () => {
      ...
      await sendToHashMock.connect(signer1).claim('a', ASSET_TYPE_COIN, ZERO_ADDRESS)
      ...
      expect((await sendToHashMock.getPayerAssetMapAssetIds(signer2Address, 'a', ASSET_TYPE_COIN, ZERO_ADDRESS)).length)
         .to.be.equal(0)
      expect(await sendToHashMock.getBeneficiaryMapAmount('a', ASSET_TYPE_COIN, ZERO_ADDRESS)).to.be.equal(0)
      expect((await sendToHashMock.getBeneficiaryMapAssetIds('a', ASSET_TYPE_COIN, ZERO_ADDRESS, signer2Address)).length)
         .to.be.equal(0)
      expect((await sendToHashMock.getBeneficiaryMapAssetIds('a', ASSET_TYPE_COIN, ZERO_ADDRESS, ownerAddress)).length)
         .to.be.equal(0)
   ...
   it ('properly handles internal mappings when invoking valid claim() for token', async () => {
      ...
      await sendToHashMock.connect(signer1).claim('a', ASSET_TYPE_TOKEN, mockToken.address)
   ...
   it ('properly handles internal mappings when invoking valid claim() for NFT', async () => {
      ...
      await sendToHashMock.connect(signer1).claim('a', ASSET_TYPE_NFT, mockNFT.address)
