...
describe("Tipping contract", async () => {
    ...
    const setupToken = async () => {
        mockToken = (await waffle.deployContract(
            owner,
            MockTokenArtifact,
            []
        )) as MockToken;
        mockToken2 = (await waffle.deployContract(
            owner,
            MockTokenArtifact,
            []
        )) as MockToken;
    ...
    const setupERC721 = async () => {
        mockNFT = (await waffle.deployContract(
            owner,
            MockNFTArtifact,
            []
        )) as MockNFT;
        mockNFT2 = (await waffle.deployContract(
            owner,
            MockNFTArtifact,
            []
        )) as MockNFT;
    ...
    const setupERC1155 = async () => {
        mockERC1155 = (await waffle.deployContract(
            owner,
            MockERC1155Artifact,
            []
        )) as MockERC1155;
        mockERC1155_2 = (await waffle.deployContract(
            owner,
            MockERC1155Artifact,
            []
        )) as MockERC1155;
    ...
    before(async () => {
        ...
        mockEAS = (await waffle.deployContract(
            owner,
            MockEASArtifact,
            []
        )) as MockEAS;
        tippingContract = (await waffle.deployContract(owner, TippingArtifact, [
            true,
            true,
    ...
    describe("Contract management", async () => {
        ...
        it("allows only admin to retrieve funds", async () => {
            ...
            await expect(tippingContract.connect(signer1).withdraw()).to.be
                .reverted;
            await expect(
            ...
            await expect(tippingContract.withdrawToken(mockToken.address)).to
                .not.be.reverted;
        });
    ...
    describe("Send native currency", () => {
        it("correctly calculates percentage fee", async () => {
            ...
            const expectedProtocolFee = weiToSend
                .mul(PAYMENT_FEE_PERCENTAGE)
                .div(PAYMENT_FEE_PERCENTAGE_DENOMINATOR);
        ...
        it("allows for sending native currency", async () => {
            ...
            expect(tippingContractBalanceAfter).to.equal(
                tippingContractBalanceBefore.add(calculatedFee)
            );
            expect(signer1BalanceAfter).to.equal(
                signer1BalanceBefore.add(weiToReceive)
            );
            ...
            const tx = await tippingContract.sendNativeTo(signer2Address, "", {
                value: weiToReceive,
            });
            ...
            expect(signer2BalanceAfter).to.equal(
                signer2BalanceBefore.add(weiToReceive)
            );
            await tippingContract.deletePublicGood(signer2Address);
        });
        ...
        it("allows sending asset to other (non-publicGood) address as batch", async () => {
            ...
            batchSendObject.forEach((call) => {
                nativeAmountToSend = nativeAmountToSend.add(
                    BigNumber.from(call.nativeAmount)
                );
            });
            const adjustedBatchSendObject = batchSendObject.map(
                ({nativeAmount, ...rest}) => rest
            );
            ...
            expect(tippingContractBalanceAfter).to.equal(
                tippingContractBalanceBefore
                    .add(nativeAmountToSend)
                    .sub(weiToReceive1)
                    .sub(weiToReceive2)
            );
            expect(sig1BalanceAfter).to.equal(
                sig1BalanceBefore.add(weiToReceive1)
            );
            expect(sig2BalanceAfter).to.equal(
                sig2BalanceBefore.add(weiToReceive2)
            );
        });
    ...
    describe("Send ERC20", () => {
        ...
        it("properly calculates fee when sending asset", async () => {
            ...
            const expectedProtocolFeeNonPGSupported = tokenToSend
                .mul(PAYMENT_FEE_PERCENTAGE)
                .div(PAYMENT_FEE_PERCENTAGE_DENOMINATOR);
        ...
        it("allows for sending unsupported ERC20 asset to other address", async () => {
            ...
            expect(sig1BalanceAfter).to.equal(
                sig1BalanceBefore.add(tokensToSend)
            );
            ...
            expect(tippingContractNativeBalanceAfter).to.equal(
                tippingContractNativeBalanceBefore.add(calculatedFeeNPG)
            );
            ...
            expect(sig2BalanceAfter).to.equal(
                sig2BalanceBefore.add(tokensToSend)
            );
        ...
        it("allows for sending supported ERC20 asset to other address", async () => {
            ...
            expect(sig1TokenBalanceAfter).to.equal(
                sig1TokenBalanceBefore.add(tokensToReceive)
            );
            expect(tippingContractTokenBalanceAfter).to.equal(
                tippingContractTokenBalanceBefore.add(calculatedFeeNPG)
            );
            ...
            expect(sig2TokenBalanceAfter).to.equal(
                sig2TokenBalanceBefore.add(tokensToSendPG)
            );
            expect(tippingContractTokenBalanceAfter).to.equal(
                tippingContractTokenBalanceAfter2
            );
        ...
        it("allows sending supported ERC20 to non-pg addresses as batch", async () => {
            ...
            batchSendObject.forEach((call) => {
                ...
                tokenAmountToSend = tokenAmountToSend.add(
                    BigNumber.from(call.amount)
                );
            });
            const adjustedBatchSendObject = batchSendObject.map(
                ({nativeAmount, ...rest}) => rest
        ...
        it("allows sending unsupported ERC20 to non-pg addresses as batch", async () => {
            ...
            expect(tippingContractBalanceAfter).to.equal(
                tippingContractBalanceBefore.add(nativeAmountToSend)
            );
        });
    ...
    describe("Send ERC721", () => {
        ...
        it("allows for sending asset to non-pg addresses as batch", async () => {
            ...
            expect(tippingContractBalanceAfter).to.equal(
                tippingContractBalanceBefore.add(dollarInWei).add(dollarInWei)
            );
        });
        ...
        it("allows for sending asset to other address (PG and Non-PG) as batch", async () => {
            ...
            expect(tippingContractBalanceAfter).to.equal(
                tippingContractBalanceBefore.add(dollarInWei)
            );
    ...
    describe("Send ERC1155", () => {
        ...
        it("allows for sending asset to other address", async () => {
            ...
            const tippingContractTokenBalanceBefore =
                await mockERC1155.balanceOf(ownerAddress, tokenToSend);
            const tippingContractTokenBalanceBefore2 =
                await mockERC1155.balanceOf(ownerAddress, tokenToSend2);
        ...
        it("allows for sending asset to other non-PG addresses as batch", async () => {
            ...
            const tippingContractNativeBalanceBefore =
                await provider.getBalance(tippingContract.address);
            const tippingContractERC1155BalanceBefore =
        ...
        it("allows for sending asset to other address (PG and Non-PG) as batch", async () => {
            ...
            expect(tippingContractNativeBalanceAfter).to.equal(
                tippingContractNativeBalanceBefore.add(dollarInWei)
            );
        ...
        it("properly sends multiple assets", async () => {
            ...
            batchSendObject.forEach((call) => {
                ...
                if (call.tokenAddress == mockToken.address)
                    mockTokenAmountToSend = mockTokenAmountToSend.add(
                        BigNumber.from(call.amount)
                    );
                if (call.tokenAddress == mockToken2.address)
                    mockToken2AmountToSend = mockToken2AmountToSend.add(
                        BigNumber.from(call.amount)
                    );
            });
            const adjustedBatchSendObject = batchSendObject.map(
            ...
            expect(tippingContractNativeBalanceAfter).to.equal(
                tippingContractNativeBalanceBefore
                    .add(dollarInWei.mul(3))
                    .add(nativeWeiToReceive.div(100))
            );
            ...
            expect(tippingContractSupportedERC20BalanceAfter).to.equal(
                tippingContractSupportedERC20BalanceBefore.add(
                    supportedErc20WeiToReceive.div(100)
                )
            );
            expect(sig1ERC1155BalanceAfter).to.equal(
                sig1ERC1155BalanceBefore.add(erc1155ToReceive)
            );
