    ...
    Batchable
{
    ...
    function sendToAnyone(
        ...
        address adjustedAssetAddress = _adjustAddress(
            _assetContractAddress,
            _assetType
        );
        uint256 msgValue = _MSG_VALUE > 0 ? _MSG_VALUE : msg.value;
        (uint256 fee, uint256 paymentValue) = _splitPayment(
            msgValue,
            _assetType
        );
        ...
        if (_assetType == AssetType.ERC20 || _assetType == AssetType.ERC1155) {
    ...
    function setStateForSendToAnyone(
        ...
        AssetLiability storage beneficiaryAsset = beneficiaryAssetMap[
            _IDrissHash
        ][_assetType][adjustedAssetAddress];
        AssetLiability storage payerAsset = payerAssetMap[msg.sender][
            _IDrissHash
        ][_assetType][adjustedAssetAddress];
        ...
        if (_assetType == AssetType.Native) {
            _checkNonZeroValue(
                _amount,
                "Transferred value has to be bigger than 0"
            );
        } else {
            _checkNonZeroValue(_amount, "Asset amount has to be bigger than 0");
            _checkNonZeroAddress(
                _assetContractAddress,
                "Asset address cannot be 0"
            );
            require(
                _isContract(_assetContractAddress),
                "Asset address is not a contract"
            );
        }

        if (
            !beneficiaryPayersMap[_IDrissHash][_assetType][
                adjustedAssetAddress
            ][msg.sender]
        ) {
            beneficiaryPayersArray[_IDrissHash][_assetType][
                adjustedAssetAddress
            ].push(msg.sender);
            beneficiaryPayersMap[_IDrissHash][_assetType][adjustedAssetAddress][
                msg.sender
            ] = true;
        }
        ...
        if (_assetType == AssetType.ERC1155) {
            AssetIdAmount memory asset = AssetIdAmount({
                id: _assetId,
                amount: paymentValue
            });
    ...
    function claim(
        ...
        bytes32 hashWithPassword = hashIDrissWithPassword(
            _IDrissHash,
            _claimPassword
        );
        ...
        address[] memory payers = beneficiaryPayersArray[hashWithPassword][
            _assetType
        ][adjustedAssetAddress];
        ...
        require(
            ownerIDrissAddr == msg.sender,
            "Only owner can claim payments."
        );

        beneficiaryAsset.amount = 0;

        for (uint256 i = 0; i < payers.length; ++i) {
            beneficiaryPayersArray[hashWithPassword][_assetType][
                adjustedAssetAddress
            ].pop();
            delete payerAssetMap[payers[i]][hashWithPassword][_assetType][
                adjustedAssetAddress
            ].assetIds[payers[i]];
            delete payerAssetMap[payers[i]][hashWithPassword][_assetType][
                adjustedAssetAddress
            ].assetIdAmounts[payers[i]];
            delete payerAssetMap[payers[i]][hashWithPassword][_assetType][
                adjustedAssetAddress
            ];
            delete beneficiaryPayersMap[hashWithPassword][_assetType][
                adjustedAssetAddress
            ][payers[i]];
            if (_assetType == AssetType.ERC721) {
                uint256[] memory assetIds = beneficiaryAsset.assetIds[
                    payers[i]
                ];
        ...
        delete beneficiaryAssetMap[hashWithPassword][_assetType][
            adjustedAssetAddress
        ];
    ...
    function balanceOf(
        ...
        AssetLiability storage asset = beneficiaryAssetMap[_IDrissHash][
            _assetType
        ][adjustedAssetAddress];
        address[] memory payers = beneficiaryPayersArray[_IDrissHash][
            _assetType
        ][adjustedAssetAddress];
        ...
        if (_assetType != AssetType.ERC1155) {
            ...
            for (uint256 i = 0; i < payers.length; ++i) {
                AssetIdAmount[] memory assetAmounts = asset.assetIdAmounts[
                    payers[i]
                ];
    ...
    function revertPayment(
        ...
        uint256[] memory assetIds = beneficiaryAssetMap[_IDrissHash][
            _assetType
        ][adjustedAssetAddress].assetIds[msg.sender];
        AssetIdAmount[] memory assetAmountIds = beneficiaryAssetMap[
            _IDrissHash
        ][_assetType][adjustedAssetAddress].assetIdAmounts[msg.sender];

        // has to be invoked after all reads required by this function, as it modifies state
        uint256 amountToRevert = setStateForRevertPayment(
            _IDrissHash,
            _assetType,
            _assetContractAddress
        );
    ...
    function setStateForRevertPayment(
        ...
        amountToRevert = payerAssetMap[msg.sender][_IDrissHash][_assetType][
            adjustedAssetAddress
        ].amount;
        AssetLiability storage beneficiaryAsset = beneficiaryAssetMap[
            _IDrissHash
        ...
        delete payerAssetMap[msg.sender][_IDrissHash][_assetType][
            adjustedAssetAddress
        ].assetIds[msg.sender];
        delete payerAssetMap[msg.sender][_IDrissHash][_assetType][
            adjustedAssetAddress
        ].assetIdAmounts[msg.sender];
        delete payerAssetMap[msg.sender][_IDrissHash][_assetType][
            adjustedAssetAddress
        ];
        beneficiaryAsset.amount -= amountToRevert;

        address[] storage payers = beneficiaryPayersArray[_IDrissHash][
            _assetType
        ][adjustedAssetAddress];

        for (uint256 i = 0; i < payers.length; ++i) {
            if (msg.sender == payers[i]) {
                delete beneficiaryPayersMap[_IDrissHash][_assetType][
                    adjustedAssetAddress
                ][payers[i]];
                if (_assetType == AssetType.ERC721) {
                    delete beneficiaryAsset.assetIds[payers[i]];
    ...
    function moveAssetToOtherHash(
        ...
        uint256[] memory assetIds = beneficiaryAssetMap[_FromIDrissHash][
            _assetType
        ][adjustedAssetAddress].assetIds[msg.sender];
        AssetIdAmount[] memory assetIdAmounts = beneficiaryAssetMap[
            _FromIDrissHash
        ][_assetType][adjustedAssetAddress].assetIdAmounts[msg.sender];
        uint256 _amount = setStateForRevertPayment(
            _FromIDrissHash,
            _assetType,
            _assetContractAddress
        );
    ...
    function isMsgValueOverride(
        bytes4 _selector
    ) internal pure override returns (bool) {
        return _selector == this.sendToAnyone.selector;
    }

    function calculateMsgValueForACall(
        bytes4,
     ...
     */
    function claimPaymentFees() external onlyOwner {
     ...
     */
    function _getAddressFromHash(
        string memory _IDrissHash
    ) internal view returns (address IDrissAddress) {
        string memory IDrissString = IIDrissRegistry(IDRISS_ADDR).getIDriss(
            _IDrissHash
        );
        ...
        _checkNonZeroAddress(
            IDrissAddress,
            "Address for the IDriss hash cannot resolve to 0x0"
        );
    }
